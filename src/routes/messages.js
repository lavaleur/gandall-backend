  ...
  try {
    ...
    for (const msg of data) {
      ...
      if (!conversations[otherId]) {
        conversations[otherId] = { other_user: otherUser, last_message: msg, unread: 0 };
      }
...
router.get('/:userId', auth, async (req, res) => {
  try {
    ...
    if (error) throw error;
    await supabase.from('messages').update({ is_read: true }).eq('receiver_id', me).eq('sender_id', other).eq('is_read', false);
